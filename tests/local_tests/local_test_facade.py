...
def validate_setting(obj: typing.Any, get_attr_func: typing.Callable[[typing.Any, ], typing.Any],
                     expected_value: typing.Any) -> bool:
...
Validator_Type = tuple[typing.Callable[[typing.Any, ], typing.Any], typing.Any]
...
@pytest.mark.parametrize(
    ...
    [
        (["-u http://127.0.0.1:8888", "-a MyUA", "--cookie=MyCookie", "--max-page=100", "--max-depth=3",
...
def test_crawler_facade_update_settings(
    ...
    validators: list[Validator_Type]
):
    ...
    for validator in validators:
        ...
        assert True is validate_setting(facade_settings, validator.get_attr_func, validator.expected_value)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["-u", "http://127.0.0.1:8888", "-d", "127.0.0.1"],
         [(lambda crawler: crawler.filter.doFilter(urlparse("http://127.0.0.1")), True),
          (lambda crawler: crawler.filter.doFilter(urlparse("http://baidu.com")), False)]),
        (["-u", "http://127.0.0.1:8888", "-D", "127.0.0.1"],
         [(lambda crawler: crawler.filter.doFilter(urlparse("http://127.0.0.1")), False),
          (lambda crawler: crawler.filter.doFilter(urlparse("http://baidu.com")), True)]),
        (["-u", "http://127.0.0.1:8888", "-i", pathlib.Path(__file__).parent / "settings.yml"],
...
def test_crawler_facade_update_crawler(
    ...
    for validator in validators:
        validator = Validator(*validator)
        assert True is validate_setting(facade_obj.crawler, validator.get_attr_func, validator.expected_value)
